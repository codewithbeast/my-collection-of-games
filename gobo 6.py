...
while run:
    ...
    if stage>=30:
        upd_ob_7 = pygame.draw.rect(game_window,red,[456,upd_y_8,13,400])
    ...
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_l:
            try:
                stage = open('save game stage','r')
                data = stage.read()
                data = int(data)
                x = open('save game x pos','r')
                data1 = x.read()
                data1 = int(data1)
                stage = data
                player_x = data1
    ...
    if stage>=31:
        upd_ob_8 = pygame.draw.rect(game_window,red,[491,upd_y_9,13,400])
        upd_y_9 += speed_1
        if upd_y_9>=200:
            upd_y_9 = -298
